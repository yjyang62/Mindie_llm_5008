...
public:
    ...
    __aicore__ inline void ComputeMm1(const AttentionCommon::RunInfo &runInfo);
...
protected:
    __aicore__ inline void Fixp(uint64_t s1gGmOffset, uint64_t s2GmOffset,uint64_t s1gL0RealSize, uint64_t s2L0RealSize, const AttentionCommon::RunInfo& runInfo);
    __aicore__ inline void ComuteL0c(uint64_t s1gL0RealSize, uint64_t s2L0RealSize, const AttentionCommon::RunInfo& runInfo);
    __aicore__ inline void LoadKeyToL0b(uint64_t s2L0Offset, uint64_t s2L1RealSize, uint64_t s2L0RealSize,
                                        const AttentionCommon::RunInfo& runInfo);
    __aicore__ inline void LoadQueryToL0a(uint64_t s1gL1Offset, uint64_t s1gL0Offset, uint64_t s1gL1RealSize,
                                          uint64_t s1gL0RealSize, const AttentionCommon::RunInfo& runInfo);
    __aicore__ inline void QueryNd2Nz(uint64_t s1gL1RealSize, uint64_t s1gL1Offset, const AttentionCommon::RunInfo& runInfo);
    __aicore__ inline void KeyNd2NzForPA(uint64_t s2L1RealSize, uint64_t s2GmOffset, const AttentionCommon::RunInfo& runInfo);
...
template <typename LIT> 
__aicore__ inline void LIMatmul<LIT>::InitMm1GlobalTensor(const GlobalTensor<int32_t> &blkTableGm, const GlobalTensor<K_T>&keyGm, const GlobalTensor<Q_T>& queryGm, const GlobalTensor<float>& mm1ResGm)
{
...
template <typename LIT>
__aicore__ inline void LIMatmul<LIT>::ComputeMm1(const AttentionCommon::RunInfo &runInfo)
{
    ...
    for (uint64_t s2GmOffset = 0; s2GmOffset < s2ProcessSize; s2GmOffset += S2_BASIC_BLOCK) {
        ...
        for (uint64_t s1gGmOffset = 0; s1gGmOffset < s1gProcessSize; s1gGmOffset += M_BASIC_BLOCK) {
            ...
            if (runInfo.isFirstS2InnerLoop && s2GmOffset == 0) {
                ...
                queryL1Mte1BufIdx_ = queryL1Mte2BufIdx_ - (CeilDiv(s1gProcessSize, M_BASIC_BLOCK) - 1 - (s1gGmOffset > 0));
            }
...
// blkNum, blkSize, N2, D
template <typename LIT> 
__aicore__ inline void LIMatmul<LIT>::KeyNd2NzForPA(uint64_t s2L1RealSize, uint64_t s2GmOffset, const AttentionCommon::RunInfo & runInfo)
{
    ...
    while(s2L1Offset < s2L1RealSize) {
        ...
        uint64_t keyGmOffset = blkTableGm_.GetValue(runInfo.bIdx * constInfo_.maxBlockNumPerBatch + s2BlkId) * constInfo_.kCacheBlockSize *
                                   constInfo_.kHeadNum * constInfo_.headDim + s2BlkOffset * constInfo_.headDim;
        ...
        nd2nzPara.dstNzC0Stride = s2L1Offset >= S2_BASIC_BLOCK_L0 ? CeilAlign(s2L1RealSize - S2_BASIC_BLOCK_L0, (uint64_t)BLOCK_CUBE) : (s2L1RealSize > S2_BASIC_BLOCK_L0 ? S2_BASIC_BLOCK_L0 : CeilAlign(s2L1RealSize, (uint64_t)BLOCK_CUBE)); // 对齐到16 单位block
...
// batch, s1, n2, g, d
template <typename LIT> 
__aicore__ inline void LIMatmul<LIT>::QueryNd2Nz(uint64_t s1gL1RealSize, uint64_t s1gGmOffset, const AttentionCommon::RunInfo & runInfo)
{
    Nd2NzParams nd2nzPara;
    nd2nzPara.ndNum = 1;
...
template <typename LIT> 
__aicore__ inline void LIMatmul<LIT>::LoadQueryToL0a(uint64_t s1gGmOffset, uint64_t s1gL1Offset, uint64_t s1gL1RealSize, uint64_t s1gL0RealSize, const AttentionCommon::RunInfo & runInfo)
{
...
template <typename LIT> 
__aicore__ inline void LIMatmul<LIT>::LoadKeyToL0b(uint64_t s2L1Offset, uint64_t s2L1RealSize, uint64_t s2L0RealSize, const AttentionCommon::RunInfo & runInfo)
{
    ...
    LoadData(keyL0_[(l0BufIdx_ % L0_BUF_NUM) * L0AB_BUFFER_OFFSET],
             keyL1_[(keyL1BufIdx_ % KEY_BUF_NUM) * KEY_BUFFER_OFFSET + keyL1Offset],
             loadData2DParams);
}

template <typename LIT> 
__aicore__ inline void LIMatmul<LIT>::ComuteL0c(uint64_t s1gL0RealSize, uint64_t s2L0RealSize, const AttentionCommon::RunInfo & runInfo) {
...
template <typename LIT> 
__aicore__ inline void LIMatmul<LIT>::Fixp(uint64_t s1gGmOffset, uint64_t s2GmOffset, uint64_t s1gL0RealSize, uint64_t s2L0RealSize, const AttentionCommon::RunInfo & runInfo) {
...
template <typename LIT> __aicore__ inline void LIMatmul<LIT>::AllocEventID()
{
