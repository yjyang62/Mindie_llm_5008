...
template <typename LIT> class LightningIndexerPreload {
public:
    __aicore__ inline LightningIndexerPreload(){};
    __aicore__ inline void Init(__gm__ uint8_t *query, __gm__ uint8_t *key, __gm__ uint8_t *weights,
                                __gm__ uint8_t *actualSeqLengthsQ, __gm__ uint8_t *actualSeqLengths,
                                __gm__ uint8_t *blockTable,
                                __gm__ uint8_t *selectedIndices,  __gm__ uint8_t *workspace,
                                const LightningIndexerTilingData *__restrict tiling, TPipe *tPipe);
    ...
    IfaServiceFlashAttentionVector<LIT> vectorService;
...
protected:
    ...
    AttentionCommon::SplitCoreInfo splitCoreInfo{};

    // ================================Init functions==================================
    __aicore__ inline void InitTilingData(const LightningIndexerTilingData *__restrict tilingData);
    __aicore__ inline void InitBuffers();
    __aicore__ inline void InitActualSeqLen(__gm__ uint8_t *actualSeqLengthsQ, __gm__ uint8_t *actualSeqLengths);
    // ================================Split Core================================
    __aicore__ inline void SplitCore(uint32_t curCoreIdx, uint32_t &coreNum, AttentionCommon::SplitCoreInfo &info);
    ...
    __aicore__ inline void ProcessBaseBlock(uint32_t loop, uint64_t s2LoopIdx, AttentionCommon::RunInfo &runInfo);
    ...
    __aicore__ inline void CalcRunInfo(uint32_t loop, uint32_t s2LoopIdx, AttentionCommon::RunInfo &runInfo);
    __aicore__ inline void DealActSeqLenIsZero(uint32_t bIdx, uint32_t n2Idx);
};

template <typename LIT>
__aicore__ inline void LightningIndexerPreload<LIT>::InitTilingData(const LightningIndexerTilingData *__restrict tilingData)
{
    ...
    constInfo.selectedCount = tilingData->selectedCount;
...
template <typename LIT>
__aicore__ inline void LightningIndexerPreload<LIT>::InitBuffers()
{
    if ASCEND_IS_AIV {
...
template <typename LIT>
__aicore__ inline void LightningIndexerPreload<LIT>::InitActualSeqLen(__gm__ uint8_t *actualSeqLengthsQ,
...
template <typename LIT>
__aicore__ inline uint32_t LightningIndexerPreload<LIT>::GetActualSeqLen(uint32_t bIdx, uint32_t actualLenDims, bool isAccumSeq, 
                                           GlobalTensor<uint32_t>& actualSeqLengthsGm, uint32_t defaultSeqLen)
{
    if (actualLenDims == 0) {
...
template <typename LIT>
__aicore__ inline void LightningIndexerPreload<LIT>::GetS1S2ActualSeqLen(uint32_t bIdx, uint32_t& actS1Size, uint32_t& actS2Size)
{
...
template <typename LIT>
__aicore__ inline uint32_t LightningIndexerPreload<LIT>::GetS2BaseBlockNumOnMask(uint32_t s1gIdx, uint32_t actS1Size, uint32_t actS2Size)
{
...
template <typename LIT>
__aicore__ inline uint32_t LightningIndexerPreload<LIT>::GetTotalBaseBlockNum()
{
...
// 多核版本，双闭区间
template <typename LIT>
__aicore__ void inline LightningIndexerPreload<LIT>::SplitCore(uint32_t curCoreIdx, uint32_t &coreNum, AttentionCommon::SplitCoreInfo& info)
{
...
template <typename LIT>
__aicore__ inline void LightningIndexerPreload<LIT>::DealActSeqLenIsZero(uint32_t bIdx, uint32_t n2Idx)
{
    if ASCEND_IS_AIV {
        if (constInfo.outputLayout == LI_LAYOUT::TND) {
            ...
            for (int s1Idx = 0; s1Idx < s1Count; s1Idx++) {
                uint64_t indiceOutOffset = (tBase + s1Idx) * constInfo.kHeadNum * constInfo.selectedCount + // T轴、s1轴偏移
                                            n2Idx * constInfo.selectedCount;                    // N2轴偏移
            ...
            for (int s1Idx = 0; s1Idx < constInfo.qSeqSize; s1Idx++) {
                // B,S1,N2,K
                uint64_t indiceOutOffset = bIdx * constInfo.qSeqSize * constInfo.kHeadNum * constInfo.selectedCount +
                                           s1Idx * constInfo.kHeadNum * constInfo.selectedCount + // B轴、S1轴偏移
                                           n2Idx * constInfo.selectedCount;                        // N2轴偏移
                vectorService.CleanInvalidOutput(indiceOutOffset);
            }
        }
...
template <typename LIT>
__aicore__ inline void LightningIndexerPreload<LIT>::Init(
    ...
    const LightningIndexerTilingData *__restrict tiling, TPipe *tPipe)
{
    ...
    // printf("bN2Start=%d, gS1Start=%d, s2Start=%d, bN2End=%d, s1GEnd=%d, s2End=%d, isLD=%d\n", splitCoreInfo.bN2Start, splitCoreInfo.gS1Start, splitCoreInfo.s2Start,
    //     splitCoreInfo.bN2End, splitCoreInfo.gS1End, splitCoreInfo.s2End, splitCoreInfo.isLD);
    ...
    if ASCEND_IS_AIV {
        ...
        indiceOutGm.SetGlobalBuffer((__gm__ int32_t *)selectedIndices);
...
template <typename LIT>
__aicore__ inline void LightningIndexerPreload<LIT>::GetBN2Idx(uint32_t bN2Idx)
{
...
template <typename LIT>
__aicore__ inline void LightningIndexerPreload<LIT>::CalcS2LoopParams(uint32_t bN2LoopIdx, uint32_t gS1LoopIdx)
{
...
template <typename LIT>
__aicore__ inline void LightningIndexerPreload<LIT>::CalcGS1LoopParams(uint32_t bN2LoopIdx)
{
...
template <typename LIT>
__aicore__ inline void LightningIndexerPreload<LIT>::CalcRunInfo(uint32_t loop, uint32_t s2LoopIdx,
                                                                AttentionCommon::RunInfo &runInfo)
{
    runInfo.loop = loop;
    ...
    runInfo.actualSingleProcessSInnerSizeAlign =
        AttentionCommon::Align((uint32_t)runInfo.actualSingleProcessSInnerSize, AttentionCommon::ConstInfo::BUFFER_SIZE_BYTE_32B);
    ...
    if (runInfo.isFirstS2InnerLoop) {
        ...
        // B,S1,N2,k/T,N2,k
        indiceOutCoreOffset = actualSeqQPrefixSum * constInfo.kHeadNum * constInfo.selectedCount +
                                runInfo.n2Idx * constInfo.selectedCount;
    }
...
template <typename LIT>
__aicore__ inline void LightningIndexerPreload<LIT>::Process()
{
    if (usedCoreNum == 0) {
        // 没有计算任务，直接清理输出
        ProcessInvalid();
...
template <typename LIT>
__aicore__ inline void LightningIndexerPreload<LIT>::ProcessInvalid()
{
    if ASCEND_IS_AIV {
        uint32_t aivCoreNum = GetBlockNum() * 2;  // 2 means c:v = 1:2
        uint64_t totalOutputSize = constInfo.batchSize * constInfo.qSeqSize * constInfo.kHeadNum * constInfo.selectedCount;
        uint64_t singleCoreSize = AttentionCommon::Align((totalOutputSize + aivCoreNum - 1) / aivCoreNum, GM_ALIGN_BYTES / sizeof(OUT_T));
...
template <typename LIT>
__aicore__ inline void LightningIndexerPreload<LIT>::ProcessMain()
{
    ...
    if ASCEND_IS_AIV {
        ...
        CrossCoreSetFlag<AttentionCommon::ConstInfo::FIA_SYNC_MODE2, PIPE_MTE2>(constInfo.syncV1C1);
        CrossCoreSetFlag<AttentionCommon::ConstInfo::FIA_SYNC_MODE2, PIPE_MTE2>(constInfo.syncV1C1);
    } else {
...
template <typename LIT>
__aicore__ inline void LightningIndexerPreload<LIT>::ProcessBaseBlock(uint32_t loop, uint64_t s2LoopIdx,
                                                                      AttentionCommon::RunInfo& runInfo)
{
...
template <typename LIT>
__aicore__ inline void LightningIndexerPreload<LIT>::ProcessDecode()
{
    if ASCEND_IS_AIV {
