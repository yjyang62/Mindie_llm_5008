...
#define INVOKE_LI_NO_KFC_OP_IMPL(templateClass, ...)                                                              \
    do {                                                                                                               \
        ...
        LI_COPY_TILING_DATA(LightningIndexerTilingData, tiling);                                              \
        op.Init(query, key, weights, actualSeqLengthsQ, actualSeqLengths, blocktable, \
            selectedIndices, user, tiling_data, &tPipe);                                                                  \
        op.Process();                                                                                              \
    } while (0)
...
__global__ __aicore__ void lightning_indexer(
    ...
    __gm__ uint8_t *selectedIndices, __gm__ uint8_t *workspace, __gm__ uint8_t *tiling)
{
    ...
    if constexpr (DT_Q == LI_TPL_FP16 && DT_K == LI_TPL_FP16 && DT_OUT == LI_TPL_INT32) {
        INVOKE_LI_NO_KFC_OP_IMPL(LightningIndexerPreload, half, half, int32_t, true,
                                LI_LAYOUT(LAYOUT_T), LI_LAYOUT(K_LAYOUT_T));
    } else {
        INVOKE_LI_NO_KFC_OP_IMPL(LightningIndexerPreload, bfloat16_t, bfloat16_t, int32_t,
                                true, LI_LAYOUT(LAYOUT_T), LI_LAYOUT(K_LAYOUT_T));
    }
#endif
}
