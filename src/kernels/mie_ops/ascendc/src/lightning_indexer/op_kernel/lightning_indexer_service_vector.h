...
using namespace LightningIndexerVecNS;
...
public:
    ...
    __aicore__ inline void ProcessVec(const AttentionCommon::RunInfo &info);
    ...
    __aicore__ inline void InitParams(const struct AttentionCommon::ConstInfo &constInfo,
                                      const LightningIndexerTilingData *__restrict tilingData);
...
private:
    ...
    LocalTensor<float> SortedBisicBlock_;
    ...
    struct AttentionCommon::ConstInfo constInfo_;
};

template <typename LIT>
__aicore__ inline void IfaServiceFlashAttentionVector<LIT>::InitBuffers(TPipe *pipe)
{
    ...
    SortedBisicBlock_ = globalTopkUb_[BASE_TOPK * 2 * 2];
    globalTopkNum_ = 0;
...
template <typename LIT>
__aicore__ inline void IfaServiceFlashAttentionVector<LIT>::InitLDBuffers(TPipe *pipe)
{
    pipe->Reset();
...
IfaServiceFlashAttentionVector<LIT>::InitParams(const struct AttentionCommon::ConstInfo &constInfo,
                                                 const LightningIndexerTilingData *__restrict tilingData)
{
...
template <typename LIT>
__aicore__ inline void IfaServiceFlashAttentionVector<LIT>::InitVec1GlobalTensor(GlobalTensor<MM1_OUT_T> mm1ResGm,
                                                                                  GlobalTensor<float> vec1ResGm,
                                                                                  GlobalTensor<int64_t> vec1ParamGm,
                                                                                  GlobalTensor<K_T> weightsGm,
                                                                                  GlobalTensor<int32_t> indiceOutGm)
...
template <typename LIT>
__aicore__ inline void IfaServiceFlashAttentionVector<LIT>::AllocEventID()
{
}

template <typename LIT>
__aicore__ inline void IfaServiceFlashAttentionVector<LIT>::FreeEventID()
{
}

template <typename LIT>
__aicore__ inline void IfaServiceFlashAttentionVector<LIT>::CleanInvalidOutput(int64_t invalidS1offset)
{
    ...
    Duplicate(idxULocal1, constInfo_.INVALID_IDX, constInfo_.selectedCount);
    outQueue_.EnQue<float>(valueULocal);
    valueULocal = outQueue_.DeQue<float>();
    LightningIndexerVecNS::CopyOut(indiceOutGm[invalidS1offset], idxULocal1, constInfo_.selectedCount);
    outQueue_.FreeTensor(valueULocal);
}

template <typename LIT>
__aicore__ inline void IfaServiceFlashAttentionVector<LIT>::ProcessVec(const AttentionCommon::RunInfo &info)
{
    ...
    for (int innerS1Idx = 0; innerS1Idx < cuS1ProcNumPerAiv; innerS1Idx++) {
        ...
        if (cuRealAcSeq > 0 && cuS2Len > 0) {
            ...
            for (int outerGidx = 0; outerGidx < outerG; outerGidx++) {
                ...
                LightningIndexerVecNS::CopyIn(mmInUb, weightsInTUb, mm1ResGm, weightsGm,
                                              mmGmOffset +
                ...
                LightningIndexerVecNS::DoScale(reduceCacheBuf[REDUCE_BANK_CONFLICT_NUM], mmInUb, weightsInUb,
                                               weightsInTUb, brcBuf, procGnum, s2BaseSize_, outerGidx);
                // confused reduceOp in DoScale
                // neednot use LightningIndexerVecNS::doReduce(mmInUb, reduceOutInner, procGnum, (s2BaseSize_+8));
            ...
            LightningIndexerVecNS::DoReduce(reduceCacheBuf[REDUCE_BANK_CONFLICT_NUM], reduceOutInner, gRedCnt,
                                            s2BaseSize_);
            ...
            PipeBarrier<PIPE_V>();
            Duplicate(sortScoreUb.template ReinterpretCast<int32_t>(), LightningIndexerVecNS::NEG_INF, cuS2LenVecAlign);
            PipeBarrier<PIPE_V>();
            ...
            if (info.actS1Size > 4) {
                ...
                LightningIndexerVecNS::SortAll(reduceOutBuff, tmpSortBuf,
                                               cuS2LenVecAlign); //  cuS2LenVecAlign <= s2BaseSize_, fill -inf
                PipeBarrier<PIPE_V>();
                LightningIndexerVecNS::MergeSort(globalTopkUb_[innerS1Idx * BASE_TOPK * 2], BASE_TOPK, reduceOutBuff,
                                                 cuS2LenVecAlign, tmpSortBuf);
            } else {
                int64_t globalTopkUbCacheIdx = (info.s2Idx - blockS2StartIdx_) % 4;
                Sort<float, true>(
                    SortedBisicBlock_[innerS1Idx * BASE_TOPK * 2 + globalTopkUbCacheIdx * s2BaseSize_ * 2],
                ...
                if (globalTopkUbCacheIdx == 3 || isS2End || info.isAllLoopEnd) {
                    LocalTensor<float> tt = SortedBisicBlock_[innerS1Idx * BASE_TOPK * 2];
                    // 前4块直接精排覆盖到globalTopkUb_
                    if (info.s2Idx - blockS2StartIdx_ < 4) {
                        MrgBisicBlock(globalTopkUb_[innerS1Idx * BASE_TOPK * 2], tt,
                                      static_cast<int64_t>(globalTopkUbCacheIdx + 1), s2BaseSize_);
                    } else { // 后面缓存在 SortedBisicBlock_, 先精排, 再merge到globalTopkUb_
                        if (globalTopkUbCacheIdx > 0) {
                            MrgBisicBlock(tmpSortBuf, tt, static_cast<int64_t>(globalTopkUbCacheIdx + 1), s2BaseSize_);
                            PipeBarrier<PIPE_V>();
                            DataCopy(SortedBisicBlock_[innerS1Idx * BASE_TOPK * 2], tmpSortBuf,
                                     (globalTopkUbCacheIdx + 1) * s2BaseSize_ * 2);
                        }
                        PipeBarrier<PIPE_V>();
                        SelectTopK(globalTopkUb_[innerS1Idx * BASE_TOPK * 2],
                                   SortedBisicBlock_[innerS1Idx * BASE_TOPK * 2], tmpSortBuf, BASE_TOPK,
            ...
            if (needCopyOutGm) {
                ...
                LightningIndexerVecNS::CopyOut(indiceOutGm[info.indiceOutOffset + cuS1Idx * constInfo_.selectedCount],
                                               idxULocal1, constInfo_.selectedCount);
                outQueue_.FreeTensor(valueULocal);
                ...
                tmpiBuff.SetValue(8, static_cast<int64_t>(info.indiceOutOffset + cuS1Idx * constInfo_.selectedCount));
                // 写入头尾判断
                // [head, tail]
                // head: 与前面规约，与前后规约
                // tail: 与后面规约
                ...
                LightningIndexerVecNS::CopyOut(vec1ParamGm[wsInfoOffset], tmpiBuff, 16);
                SetWaitFlag<HardEvent::V_MTE3>(HardEvent::V_MTE3);
                LightningIndexerVecNS::CopyOut(vec1ResGm[wsOffset], globalTopkUb_[innerS1Idx * BASE_TOPK * 2],
                                               2 * BASE_TOPK);
                SetWaitFlag<HardEvent::MTE3_V>(HardEvent::MTE3_V);
            }
        } else if (cuRealAcSeq <= 0) {
            CleanInvalidOutput(info.indiceOutOffset + cuS1Idx * constInfo_.selectedCount);
        }
    }
    ...
    if (LAYOUT_T == LI_LAYOUT::BSND) {
        ...
        if (invalidS1Num > 0 && isS1LoopEnd && blockS2StartIdx_ == 0) {
            ...
            for (int innerS1Idx = 0; innerS1Idx < s1NumPerAiv; innerS1Idx++) {
                CleanInvalidOutput(info.indiceOutOffset + (s1OffsetPerAiv + innerS1Idx) * constInfo_.selectedCount);
            }
        }
...
template <typename LIT>
__aicore__ inline void IfaServiceFlashAttentionVector<LIT>::ProcessLD()
{
    ...
    DataCopyPad(curParamUbHead, vec1ParamGm[tmpCubeId * s1BaseSize_ * 2 * paramNum_],
                {1, static_cast<uint16_t>(s1BaseSize_ * 2 * paramNum_ * sizeof(int64_t)), 0,
                 0},
                {true, 0, 0, 0});
    ...
    for (uint32_t innerS1Idx = s1LdStartIdx; innerS1Idx < s1LdStartIdx + s1VecNum; innerS1Idx++) {
        ...
        DataCopyPad(curParamUbHead, vec1ParamGm[wsInfoOffset], {1, static_cast<uint16_t>(2 * paramNum_ * sizeof(int64_t)), 0, 0},
                    {true, 0, 0, 0});
        SetWaitFlag<HardEvent::MTE2_S>(HardEvent::MTE2_S);
        ...
        while (needFd == 1) {
            ...
            DataCopyPad(curParamUbHead, vec1ParamGm[wsInfoOffset],
                        {1, static_cast<uint16_t>(2 * paramNum_ * sizeof(int64_t)), 0, 0},
                        {true, 0, 0, 0});
            SetWaitFlag<HardEvent::MTE2_S>(HardEvent::MTE2_S);
            needFd = curParamUbHead.GetValue(0);
            isS2End = curParamUbHead.GetValue(4);
        ...
        DataCopyPad(indiceOutGm[outOffset], idxULocal1,
                    {1, static_cast<uint16_t>(constInfo_.selectedCount * sizeof(int32_t)), 0, 0});
    }
}
#endif